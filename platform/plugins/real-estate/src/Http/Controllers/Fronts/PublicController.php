...
class PublicController extends BaseController
{
    ...
    public function getPropertyValuation($id, Request $request)
    {
        $property = Property::query()->where('id', $id)->firstOrFail();
        
        $recalculate = $request->boolean('recalculate', false);
        
        $valuationService = app(\Botble\RealEstate\Services\PropertyValuationService::class);
        $valuation = $valuationService->getValuation($property, $recalculate);
    ...
    public function getRecommendations(Request $request)
    {
        ...
        $recommendationService = app(\Botble\RealEstate\Services\PropertyRecommendationService::class);
        $recommendations = $recommendationService->getRecommendations($limit);

        if ($request->expectsJson()) {
            return $this->httpResponse()
                ->setData(['properties' => $recommendations]);
        }

        return Theme::scope('real-estate.properties', [
            'properties' => $recommendations,
            'title' => trans('plugins/real-estate::real-estate.recommended_properties'),
